...
void Disciplina::setProfessor(const Pessoa* prof) { this->professor = prof; }
...
void Disciplina::removerAluno(Pessoa* aluno) { this->alunos.remove(aluno); }
...
void Disciplina::removerAluno(unsigned long cpf) {
  ...
  if (it != this->alunos.end()) alunos.erase(it);
}

std::list<Pessoa*>&
Disciplina::getAlunos() {  // retornamos uma referência para a lista, o que
                           // custa mais barato
  return alunos;
}